...
//create course
const createCourseIntoDB = async (course) => {
    ...
    if (prerequisiteCourses?.length) {
        const invalidIds = prerequisiteCourses.filter((item) => !mongoose.Types.ObjectId.isValid(item.course?.toString()));
...
//get all courses
const getAllCourcesFromDB = async (query = {}) => {
    const parsed = parseListQuery(query, {
        searchableFields: ["title", "prefix"],
    });
    const { meta, data: courses } = await paginate(CourseModel, parsed, (q) => q.populate(buildNestedPopulate("prerequisiteCources.course", 3)));
    return { meta, courses };
};
//get single course by id
const getSingleCourseByIdFromDB = async (id) => {
...
//update course by id
const updateCourseByIdInDB = async (id, updatedData) => {
    ...
    const updateDoc = {};
    const setData = normalizeMongoUpdatePayload(remainingData);
    if (Object.keys(setData).length > 0) {
        updateDoc.$set = setData;
    }
    if (prerequisiteCources?.length) {
        const invalidIds = prerequisiteCources.filter((item) => !mongoose.Types.ObjectId.isValid(item.course?.toString()));
        if (invalidIds.length) {
            throw new AppError("Invalid prerequisite course ids", 400);
        }
        const selfIncluded = prerequisiteCources.some((item) => item.course.toString() === id);
        if (selfIncluded) {
        ...
        if (toAdd.length > 0) {
            const toAddIds = toAdd.map((item) => new mongoose.Types.ObjectId(item.course.toString()));
            const existing = await CourseModel.find({ _id: { $in: toAddIds }, isDeleted: false }, { _id: 1 });
    ...
    const updatedCourse = await CourseModel.findOneAndUpdate({ _id: id, isDeleted: false }, updateDoc, { returnDocument: "after" });
    return updatedCourse;
};
//delete course by id
const deleteCourseByIdInDB = async (id) => {
    const deletedCourse = await CourseModel.findByIdAndUpdate({ _id: id, isDeleted: false }, { isDeleted: true }, { returnDocument: "after" });
    return deletedCourse;
};
//assign cources to faculties
const assignCourcesToFacultiesInDB = async (courseId, facultyData) => {
    ...
    const invalidIds = facultyIds.filter((id) => !mongoose.Types.ObjectId.isValid(id.toString()));
    if (invalidIds.length) {
    ...
    const assignedCources = await AssignCourcesToFacultiesModel.findOneAndUpdate({ courseId: new mongoose.Types.ObjectId(courseId) }, {
...
const unassignCourcesFromFacultiesInDB = async (courseId, facultyIds) => {
    ...
    const unassignedCources = await AssignCourcesToFacultiesModel.findOneAndUpdate({ courseId: new mongoose.Types.ObjectId(courseId) }, { $pull: { faculties: { $in: uniqueFacultyIds } } }, { returnDocument: "after" });
...
const getSingleAssignedCourseToFacultyInDB = async (facultyId) => {
...
const getAssignedFacultyByCourseIdInDB = async (courseId) => {
    const parsed = parseListQuery({ courseId }, {
        searchableFields: ["courseId"],
        baseFilter: { courseId: courseId },
    });
    const { meta, data: facultiesData } = await paginate(AssignCourcesToFacultiesModel, parsed, (q) => q.populate("faculties"));
