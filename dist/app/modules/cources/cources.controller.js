...
// get all users-GET
const getAllCources = catchAsync(async (req, res, next) => {
...
const getCourseById = catchAsync(async (req, res, next) => {
    ...
    const result = await CourseService.getSingleCourseByIdFromDB(courseId);
    if (result) {
        // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
...
const updateCourseInfo = catchAsync(async (req, res, next) => {
    const courseId = checkCommonValidation.validateId(req.params.id, next);
    const updatedData = req.body; // Get updated course data from the request body
    const result = await CourseService.updateCourseByIdInDB(courseId, updatedData);
    if (result) {
        // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
...
const deleteCourseById = catchAsync(async (req, res, next) => {
    const courseId = checkCommonValidation.validateId(req.params.id, next);
    const result = await CourseService.deleteCourseByIdInDB(courseId);
    if (result) {
        // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
...
const assignCourcesToFaculties = catchAsync(async (req, res, next) => {
    const courseId = checkCommonValidation.validateId(req.params.courseId, next);
    const facultyData = req.body;
    const result = await CourseService.assignCourcesToFacultiesInDB(courseId, facultyData);
    if (result) {
        sendResponse(res, {
            statusCode: 200,
            success: true,
...
const getAssignedFacultyByCourseId = catchAsync(async (req, res, next) => {
    const courseId = checkCommonValidation.validateId(req.params.courseId, next);
    const result = await CourseService.getAssignedFacultyByCourseIdInDB(courseId);
    if (result) {
        sendResponse(res, {
            statusCode: 200,
            success: true,
...
const unassignCourcesFromFaculties = catchAsync(async (req, res, next) => {
    const courseId = checkCommonValidation.validateId(req.params.courseId, next);
    const facultyIds = Array.isArray(req.body) ? req.body : req.body.faculties;
    const result = await CourseService.unassignCourcesFromFacultiesInDB(courseId, facultyIds);
    if (result) {
        sendResponse(res, {
            statusCode: 200,
            success: true,
...
const getAllCoursesAssignedToFaculty = catchAsync(async (_req, res, next) => {
...
const getSingleAssignedCourseToFaculty = catchAsync(async (req, res, next) => {
    const facultyId = checkCommonValidation.validateId(req.params.facultyId, next);
    const result = await CourseService.getSingleAssignedCourseToFacultyInDB(facultyId);
    if (result) {
        sendResponse(res, {
            statusCode: 200,
            success: true,
