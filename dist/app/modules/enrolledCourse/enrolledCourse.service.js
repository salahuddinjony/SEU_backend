...
import { evaluateCourseMarks, mergeAssessmentMarks, } from "./utils/enrolledCourse.marksCalculation.js";
const createEnrolledCourseIntoDB = async (userId, payload) => {
    ...
    try {
        ...
        const offeredCourse = await OfferedCourseModel.findOne({
            ...
            .select("_id maxCapacity semseterRegistration academicDepartment course faculty")
            .session(session)
            .lean();
        if (!offeredCourse) {
        ...
        const [student, semesterRegistration, academicDepartment, course, faculty] = await Promise.all([
        ...
        if (newTotalCredit > semesterRegistration.maxCredit) {
            throw new AppError("Taken course credit is greater than the max credit of the semester registration", 400);
        }
        ...
        const isEnrolled = await EnrolledCourseModel.findOne({
            ...
            .lean();
        if (isEnrolled) {
            throw new AppError("This student is already enrolled in this offered course", 400);
        }
        ...
        throw new AppError(error?.message || "Failed to create enrolled course", error?.statusCode || 500);
    }
    finally {
        await session.endSession();
    }
};
const getAllEnrolledCoursesFromDB = async (query = {}) => {
    ...
    const { meta, data: enrolledCourses } = await paginate(EnrolledCourseModel, parsed, (q) => q
        .populate("academicSemester academicDepartment academicFaculty course faculty")
        .populate({
...
const getEnrolledCourseByIdFromDB = async (enrolledCourseId) => {
...
const getMyEnrolledCoursesFromDB = async (user, query = {}) => {
...
const updateEnrolledCourseByIdInDB = async (enrolledCourseId, updatedData) => {
    const { student, semesterRegistration, offeredCourse, courseMarks } = updatedData;
    console.log({
        studentId: student,
    ...
    if (!student || !semesterRegistration || !offeredCourse) {
        throw new AppError("student, semesterRegistration, offeredCourse are required", 400);
    }
    const enrolledCourse = await EnrolledCourseModel.findOne({
        _id: enrolledCourseId,
        student: student,
    ...
    const mergedMarks = mergeAssessmentMarks(enrolledCourse.courseMarks, courseMarks);
    const { total, grade, gradePoints, status, isCourseCompleted } = evaluateCourseMarks(mergedMarks);
    ...
    const updatedEnrolledCourse = await EnrolledCourseModel.findByIdAndUpdate(enrolledCourseId, { $set: normalizedPayload }, { new: true, runValidators: true });
    return updatedEnrolledCourse;
};
const deleteEnrolledCourseByIdFromDB = async (courseId) => {
    return {};
};
...
//# sourceMappingURL=enrolledCourse.service.js.map