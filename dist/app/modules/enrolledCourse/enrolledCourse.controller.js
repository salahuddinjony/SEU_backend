...
const createEnrolledCourse = catchAsync(async (req, res, next) => {
    ...
    const result = await EnrolledCourseService.createEnrolledCourseIntoDB(userId, enrolledCourseData);
    if (result) {
...
const getAllEnrolledCourses = catchAsync(async (req, res, next) => {
    ...
    const result = await EnrolledCourseService.getAllEnrolledCoursesFromDB(query);
    if (result) {
        // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
...
// get user by ID-GET
const getEnrolledCourseById = catchAsync(async (req, res, next) => {
    const courseId = checkCommonValidation.validateId(req.params.id, next);
    const result = await EnrolledCourseService.getEnrolledCourseByIdFromDB(courseId);
    if (result) {
        // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
...
const updateEnrolledCourseById = catchAsync(async (req, res, next) => {
    const enrolledCourseId = checkCommonValidation.validateId(req.params.id, next);
    const updatedData = req.body; // Get updated course data from the request body
    const result = await EnrolledCourseService.updateEnrolledCourseByIdInDB(enrolledCourseId, updatedData);
    if (result) {
        // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
...
// delete user-DELETE
const deleteEnrolledCourseById = catchAsync(async (req, res, next) => {
    const courseId = checkCommonValidation.validateId(req.params.id, next);
    const result = await EnrolledCourseService.deleteEnrolledCourseByIdFromDB(courseId);
    if (result) {
        // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
...
//# sourceMappingURL=enrolledCourse.controller.js.map