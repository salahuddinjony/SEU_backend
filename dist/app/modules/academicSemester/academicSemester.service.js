...
const createSemesterIntoDB = async (semesterData) => {
...
const getAllSemestersFromDB = async (query = {}) => {
...
const getSemesterByIdFromDB = async (id) => {
    const semester = await AcademicSemesterModel.findById(id);
    return semester;
};
// update semester info
const updateSemesterInfoInDB = async (id, updatedData) => {
    ...
    const updatedSemester = await AcademicSemesterModel.findByIdAndUpdate(id, normalizeMongoUpdatePayload(updatedData), { returnDocument: "after" });
...
const deleteSemesterFromDB = async (id) => {
    ...
    try {
        ...
        const deletedSemester = await AcademicSemesterModel.findOneAndUpdate({ _id: id, isDeleted: false }, { isDeleted: true }, { new: true, session });
        if (!deletedSemester) {
...
const restoreDeletedSemestersInDB = async () => {
    ...
    await AcademicSemesterModel.updateMany({ isDeleted: true }, // Filter to find all documents that are currently marked as deleted
    { isDeleted: false });
...
const restoreDeletedSemesterByIdInDB = async (id) => {
...
//# sourceMappingURL=academicSemester.service.js.map