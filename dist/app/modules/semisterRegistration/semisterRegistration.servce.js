...
const createSemesterRegistrationIntoDB = async (semesterRegistration) => {
    ...
    const { minCredit, maxCredit } = getSemesterRegistrationCreditsByCode(academicSemesterCode);
    ...
    const [existingSemesterRegistration, existingRegistration] = await Promise.all([
    ...
    if (existingSemesterRegistration) {
        throw new AppError("Semister registration already exists for this academic semester", 400);
    }
    //then checck is there already any registration status 'UPCOMING' or 'ONGOING' if not then throw an error
    if (existingRegistration) {
        throw new AppError("There are already some registrations in the UPCOMING or ONGOING status", 400);
    }
    return await SemesterRegistrationModel.create(semesterRegistration);
};
//get all semister registration
const getAllSemesterRegistrationFromDB = async (query = {}) => {
    const parsed = parseListQuery(query, {
        searchableFields: ["status"],
        baseFilter: { isDeleted: false },
    });
    const { meta, data: semesterRegistrations } = await paginate(SemesterRegistrationModel, parsed, (q) => q.populate("academicSemester"));
    return { meta, semesterRegistrations };
};
// get single semister registration by id
const getSemesterRegistrationByIdFromDB = async (id) => {
...
const updateSemesterRegistrationInfoInDB = async (id, updatedData) => {
    ...
    const updated = await SemesterRegistrationModel.findByIdAndUpdate(id, normalizeMongoUpdatePayload(updatedData), {
...
const deleteSemesterRegistrationFromDB = async (id) => {
    ...
    try {
        ...
        await OfferedCourseModel.updateMany({
            ...
            isDeleted: false,
        }, { isDeleted: true }, { session });
        const result = await SemesterRegistrationModel.findByIdAndUpdate(id, { isDeleted: true }, { returnDocument: "after", session });
...
//# sourceMappingURL=semisterRegistration.servce.js.map