...
const getSemesterRegistrationById = catchAsync(async (req, res, next) => {
    const semisterRegistrationId = checkCommonValidation.validateId(req.params.id, next);
    const result = await SemesterRegistrationService.getSemesterRegistrationByIdFromDB(semisterRegistrationId);
    if (result) {
        // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
...
const updateSemesterRegistrationInfo = catchAsync(async (req, res, next) => {
    const semisterRegistrationId = checkCommonValidation.validateId(req.params.id, next);
    const updatedData = req.body; // Get updated semister registration data from the request body
    const result = await SemesterRegistrationService.updateSemesterRegistrationInfoInDB(semisterRegistrationId, updatedData);
    if (result) {
        // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
...
// delete user-DELETE
const deleteSemesterRegistrationById = catchAsync(async (req, res, next) => {
    const semisterRegistrationId = checkCommonValidation.validateId(req.params.id, next);
    const result = await SemesterRegistrationService.deleteSemesterRegistrationFromDB(semisterRegistrationId);
    if (result) {
        // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
...
//# sourceMappingURL=semisterRegistration.controller.js.map