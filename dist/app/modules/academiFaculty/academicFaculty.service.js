...
const createAcademicFacultyIntoDB = async (facultyData) => {
    const session = await mongoose.startSession();
    try {
        session.startTransaction();
        const startFacultyId = "F-";
        const facultyIdCounter = await CounterService.createOrFindCounterIntoDB(startFacultyId, "academicFaculty", session);
        if (!facultyIdCounter) {
...
export const getAllAcademicFacultiesFromDB = async (query = {}) => {
...
const getAcademicFacultyByIdFromDB = async (id) => {
    const faculty = await AcademicFacultyModel.findById(id);
    return faculty;
};
// update faculty info
const updateAcademicFacultyInfoInDB = async (id, updatedData) => {
    ...
    const updatedFaculty = await AcademicFacultyModel.findByIdAndUpdate(id, normalizeMongoUpdatePayload(updatedData), { returnDocument: "after" });
    return updatedFaculty;
};
// delete faculty from database
const deleteAcademicFacultyFromDB = async (id) => {
    const session = await mongoose.startSession();
    try {
        session.startTransaction();
        const deletedFaculty = await AcademicFacultyModel.findOneAndUpdate({ _id: id, isDeleted: false }, { isDeleted: true }, { returnDocument: "after", session });
        if (!deletedFaculty) {
            await session.abortTransaction();
            return null;
        }
        ...
        if (departmentIds.length > 0) {
            await AcademicDeptModel.updateMany({ _id: { $in: departmentIds } }, { isDeleted: true }, { session });
...
const restoreDeletedAcademicFacultiesInDB = async () => {
    ...
    try {
        ...
        await AcademicFacultyModel.updateMany({ _id: { $in: facultyIds } }, { isDeleted: false }, { session });
        await session.commitTransaction();
...
//# sourceMappingURL=academicFaculty.service.js.map