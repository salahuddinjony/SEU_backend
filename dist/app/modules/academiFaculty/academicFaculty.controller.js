...
const getAcademicFacultyById = catchAsync(async (req, res, next) => {
    const facultyId = checkCommonValidation.validateId(req.params.id, next);
    const result = await AcademicFacultyService.getAcademicFacultyByIdFromDB(facultyId);
    if (result) { // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
            success: true,
...
const updateAcademicFacultyInfo = catchAsync(async (req, res, next) => {
    const facultyId = checkCommonValidation.validateId(req.params.id, next);
    ...
    const result = await AcademicFacultyService.updateAcademicFacultyInfoInDB(facultyId, updatedData);
    if (result) { // Check if result is not null or undefined
        sendResponse(res, {
            statusCode: 200,
            success: true,
...
const restoreDeletedAcademicFaculties = catchAsync(async (req, res, next) => {
...
const getAllDeletedAcademicFaculties = catchAsync(async (req, res, next) => {
...
//# sourceMappingURL=academicFaculty.controller.js.map