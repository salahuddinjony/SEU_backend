...
const createAcademicDeptIntoDB = async (academicDeptData) => {
...
const getAllAcademicDeptsFromDB = async (query = {}) => {
    const parsed = parseListQuery(query, {
        searchableFields: ["name"],
    });
    const { meta, data: academicDepts } = await paginate(AcademicDeptModel, parsed, (q) => q.populate("academicFaculty"));
    return {
        total: meta.total,
        page: meta.page,
        limit: meta.limit,
...
const getAcademicDeptByIdFromDB = async (id) => {
    const academicDept = await AcademicDeptModel.findById(id).populate("academicFaculty");
    return academicDept;
};
// update department info
const updateAcademicDeptInfoInDB = async (id, updatedData) => {
    ...
    const updatedAcademicDept = await AcademicDeptModel.findByIdAndUpdate(id, normalizeMongoUpdatePayload(updatedData), { returnDocument: "after" }).populate("academicFaculty");
...
const deleteAcademicDeptFromDB = async (id) => {
    const session = await mongoose.startSession();
    try {
        session.startTransaction();
        const deletedAcademicDept = await AcademicDeptModel.findOneAndUpdate({ _id: id, isDeleted: false }, { isDeleted: true }, { new: true, session });
...
const restoreDeletedAcademicDeptsInDB = async () => {
    ...
    await AcademicDeptModel.updateMany({ isDeleted: true }, // Filter to find all documents that are currently marked as deleted
    { isDeleted: false });
...
//# sourceMappingURL=academicDept.service.js.map