...
export const guradianSchema = new Schema({
    fatherName: {
        type: String,
        trim: true,
    ...
    fatherContactNo: {
        ...
        validate: {
            ...
            message: "{VALUE} is not a valid Bangladeshi contact number.",
        },
        trim: true,
...
export const localGuardianSchema = new Schema({
    name: {
        type: String,
        trim: true,
...
const studentSchema = new Schema({
    ...
    toJSON: {
        // This will ensure that virtuals are included when converting documents to JSON
        virtuals: true,
    },
});
// Exclude password and isDeleted fields when converting to JSON
applyExcludeFields(studentSchema, ["isDeleted"]);
...
studentSchema.post("save", function (doc, next) {
    ...
    doc.password = undefined; // This will remove the password field from the document before it is sent back in the response, ensuring that the hashed password is not exposed in any API responses or logs.
