...
const getAllStudentsFromDB = async (query) => {
    ...
    const { meta, data: students } = await paginate(StudentModel, parsed, (q) => q
        .populate("user")
...
// }
//update info
const updateStudentInfoInDB = async (id, updatedData) => {
    ...
    const updatedStudentData = normalizeMongoUpdatePayload(updatedData);
...
const deleteStudentFromDB = async (id) => {
    ...
    try {
        ...
        const deletedStudent = await StudentModel.findOneAndUpdate({ _id: id, isDeleted: false }, { isDeleted: true }, {
        ...
        await UserModel.findByIdAndUpdate(deletedStudent.user, { isDeleted: true }, { session });
...
const restoreDeletedStudentsInDB = async () => {
    ...
    try {
        ...
        const validStudents = deletedStudents.filter((student) => {
            const semester = student.admissionSemester;
            const dept = student.academicDept;
            return (semester &&
                !semester.isDeleted &&
                dept &&
                !dept.isDeleted &&
        ...
        if (validUserIds.length > 0) {
            await UserModel.updateMany({ _id: { $in: validUserIds } }, { isDeleted: false }, { session });
        }
        ...
        await StudentModel.updateMany({ _id: { $in: validStudentIds } }, { isDeleted: false }, { session });
