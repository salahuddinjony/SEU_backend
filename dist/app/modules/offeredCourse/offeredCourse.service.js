import { parseListQuery } from "../../builder/queryBuilder.js";
...
import { assertNoFacultyScheduleConflict, assertOfferedCourseDatesWithinSemester, } from "../../utils/offeredCourseScheduleValidation.js";
...
const createOfferedCourseIntoDB = async (payload) => {
    ...
    assertOfferedCourseDatesWithinSemester(payload.startDate, payload.endDate, semesterReg);
...
const getMyOfferedCoursesFromDB = async (query = {}, user) => {
    ...
    const semesterRegistrationCollection = SemesterRegistrationModel.collection.name;
...
const getSingleOfferedCourseByIdFromDB = async (id) => {
    return await OfferedCourseModel.findOne({ _id: id, isDeleted: false });
};
//update offered course by id
const updateOfferedCourseByIdInDB = async (id, updatedData) => {
    ...
    if (!semesterReg) {
        throw new AppError("Invalid semester registration for this offered course", 400);
    }
    ...
    const scheduleFieldsTouched = updatedData.startDate !== undefined ||
...
const deleteOfferedCourseByIdInDB = async (id) => {
    ...
    return await OfferedCourseModel.findByIdAndUpdate(id, { isDeleted: true }, { returnDocument: "after" });
};
...
//# sourceMappingURL=offeredCourse.service.js.map