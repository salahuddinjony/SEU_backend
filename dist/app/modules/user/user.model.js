...
import { userRoleEnum, userStatusEnum } from "./user.constant.js";
...
export const userSchema = new Schema({
    ...
    role: {
        ...
        enum: {
            values: userRoleEnum,
            message: "Role must be one of admin, student, faculty, or super_admin",
        },
    },
    isDeleted: {
        type: Boolean,
...
// custom methods
// Login / user identifier validation (accepts either id or email)
userSchema.statics.isUserIdValid = async function (identifier, password, checkPassword = true, checkIsBlocked = true) {
    const user = await UserModel.findOne({
        $or: [{ id: identifier }, { email: identifier }],
        isDeleted: false,
    }).select("+password status role needsPasswordReset id passwordChangedAt email");
    if (!user) {
...
userSchema.statics.isPasswordChanged = function (passwordChangedAt, iat) {
