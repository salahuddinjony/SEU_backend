...
const FacultyModel = UserModel;
const AcademicSemesterModel = UserModel;
const AcademicDeptModel = UserModel;
// Service function to create a user in the database
const createStudentIntoDB = async (password, StudentData, file, status = "in-progress") => {
    ...
    try {
        ...
        userData.id = await UserUtils.generateUserId("student", session);
        ...
        throw new AppError(error instanceof Error ? error.message : "Failed to create student user", 500);
    }
    finally {
...
const createFacultyIntoDB = async (password, FacultyData, file) => {
    // Simulating saving the user data to the database
    const userData = {};
    const resolved = UserUtils.resolveNewUserPassword(password);
    userData.password = resolved.password;
    ...
    try {
        ...
        await session.abortTransaction();
        throw new AppError(error instanceof Error ? error.message : "Failed to create faculty user", 500);
    }
    finally {
        await removeUploadedLocalFile(file?.path);
        await session.endSession();
...
const createAdminIntoDB = async (password, AdminData, file) => {
    // Simulating saving the user data to the database
    const userData = {};
    const resolved = UserUtils.resolveNewUserPassword(password);
    userData.password = resolved.password;
    ...
    try {
        ...
        await session.abortTransaction();
        throw new AppError(error instanceof Error ? error.message : "Failed to create admin user", 500);
    }
    finally {
        await removeUploadedLocalFile(file?.path);
        await session.endSession();
...
const updateUserInfoInDB = async (id, updatedData) => {
    const payload = typeof updatedData === "object" &&
        updatedData !== null &&
        !Array.isArray(updatedData)
        ? normalizeMongoUpdatePayload(updatedData)
        : updatedData;
    const updatedUser = await UserModel.findOneAndUpdate({ _id: id, isDeleted: false }, payload, {
...
const deleteUserFromDB = async (id) => {
    ...
    try {
        session.startTransaction();
        const deletedUser = await UserModel.findOneAndUpdate({ _id: id, isDeleted: false }, { isDeleted: true }, { returnDocument: "after", session });
        if (!deletedUser) {
            await session.abortTransaction();
            return null;
        }
        // Keep user and related student soft-delete in the same transaction.
        await StudentModel.findOneAndUpdate({ user: deletedUser._id, isDeleted: false }, { isDeleted: true }, { session });
        // 3. Keep user and related admin soft-delete in the same transaction.
        await AdminModel.findOneAndUpdate({ user: deletedUser._id, isDeleted: false }, { isDeleted: true }, { session });
        // 4. Keep user and related faculty soft-delete in the same transaction.
        await FacultyModel.findOneAndUpdate({ user: deletedUser._id, isDeleted: false }, { isDeleted: true }, { session });
        await session.commitTransaction();
        return deletedUser;
    }
    catch {
...
const restoreDeletedUsersInDB = async () => {
    ...
    try {
        session.startTransaction();
        // 1. Get deleted users
        const deletedUsers = await UserModel.find({ isDeleted: true }, { _id: 1 }).session(session);
        ...
        // 2. Restore users
        const restoredUsers = await UserModel.updateMany({ _id: { $in: userIds } }, { isDeleted: false }, { session });
        // 3. Restore ONLY related students
        await StudentModel.updateMany({ user: { $in: userIds }, isDeleted: true }, { isDeleted: false }, { session });
...
const getMyProfileFromDB = async (user) => {
    ...
    else if (user.role === UserRole.USER || user.role === UserRole.SUPER_ADMIN) {
...
const createPortalUserIntoDB = async (payload) => {
    const existingUser = await UserModel.findOne({ email: payload.email, isDeleted: false });
    if (existingUser)
        throw new AppError("Email is already registered", 409);
    const id = await UserUtils.generateUserId("user");
...
const createPortalAdminIntoDB = async (payload) => {
    const existingUser = await UserModel.findOne({ email: payload.email, isDeleted: false });
    if (existingUser)
        throw new AppError("Email is already registered", 409);
    const id = await UserUtils.generateUserId("admin");
    return UserModel.create({
        ...payload,
        id,
        role: UserRole.ADMIN,
...
export const UserService = {
    ...
    createFacultyIntoDB,
    createAdminIntoDB,
    createPortalUserIntoDB,
    createPortalAdminIntoDB,
    getAllUsersFromDB,
    getUserByIdFromDB,
    updateUserInfoInDB,
    deleteUserFromDB,
